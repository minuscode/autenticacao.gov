...
*****************************************************************************************/
  ...
  APL_CertifStatus APL_OcspResponse::getResponse(CByteArray *response)
  {
    ...
    if (m_response)
    {
      if ((m_status == APL_CERTIF_STATUS_VALID_OCSP || m_status == APL_CERTIF_STATUS_REVOKED || m_status == APL_CERTIF_STATUS_SUSPENDED) && CTimestampUtil::checkTimestamp(m_validity, CSC_VALIDITY_FORMAT))
      {
        if (response)
          *response = *m_response;
